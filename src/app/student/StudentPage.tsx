...
const BELT_COLORS: Record<Belt, { bg: string; text: string; border: string }> = {
  ...
  Orange: { bg: "#e8650a", text: "#fff",    border: "#c45508" },
...
const BELT_ORDER: Belt[] = ["White","Yellow","Orange","Green","Blue","Brown","Black"];
...
const SAMPLE_STUDENTS: Student[] = [
  ...
  { id:7,  firstName:"Dev",      lastName:"Kapoor",    age:11, belt:"Orange", state:"Delhi",       sensei:"Raj Nair",     campsCount:1,  eventsCount:2,  isChampion:false },
...
const BELT_AURA: Record<Belt, {
  ringColor:   string;   // main ring colour
  glowColor:   string;   // box-shadow glow
  animation:   string;   // CSS animation name
  extraRings?: { color: string; delay: string; size: string }[];
}> = {
  White: {
    ringColor:  "rgba(255,255,255,0.5)",
    glowColor:  "rgba(255,255,255,0.15)",
    animation:  "aura-shimmer",
  },
  Yellow: {
    ringColor:  "#f5c518",
    glowColor:  "rgba(245,197,24,0.4)",
    animation:  "aura-pulse",
    extraRings: [{ color:"rgba(245,197,24,0.25)", delay:"0.4s", size:"78px" }],
  },
  Orange: {
    ringColor:  "#e8650a",
    glowColor:  "rgba(232,101,10,0.5)",
    animation:  "aura-ember",
    extraRings: [
      { color:"rgba(232,101,10,0.3)", delay:"0s",   size:"80px" },
      { color:"rgba(255,140,50,0.15)",delay:"0.6s", size:"88px" },
    ],
  },
  Green: {
    ringColor:  "#3a8c3a",
    glowColor:  "rgba(58,140,58,0.4)",
    animation:  "aura-breathe",
    extraRings: [{ color:"rgba(58,140,58,0.2)", delay:"0.8s", size:"82px" }],
  },
  Blue: {
    ringColor:  "#2255aa",
    glowColor:  "rgba(34,85,170,0.5)",
    animation:  "aura-ripple",
    extraRings: [
      { color:"rgba(34,85,170,0.25)", delay:"0s",   size:"80px" },
      { color:"rgba(34,85,170,0.12)", delay:"0.5s", size:"90px" },
    ],
  },
  Brown: {
    ringColor:  "#7a4520",
    glowColor:  "rgba(122,69,32,0.4)",
    animation:  "aura-earthpulse",
    extraRings: [{ color:"rgba(122,69,32,0.2)", delay:"1s", size:"80px" }],
  },
  Black: {
    ringColor:  "#555",
    glowColor:  "rgba(180,180,180,0.25)",
    animation:  "aura-rotate",
    extraRings: [
      { color:"rgba(255,255,255,0.08)", delay:"0s",   size:"80px" },
      { color:"rgba(255,255,255,0.04)", delay:"0.3s", size:"90px" },
    ],
  },
};

// ── Belt card border config — PUBG-style vertical panel ──────────────────────
const BELT_PANEL: Record<Belt, {
  gradient: string;
  pattern:  string;   // "solid" | "dashes" | "dots" | "zigzag" | "double"
  glow:     string;
}> = {
  White:  {
    gradient: "linear-gradient(to bottom, #e8e8e8, #aaa, #e8e8e8)",
    pattern:  "solid",
    glow:     "rgba(255,255,255,0.15)",
  },
  Yellow: {
    gradient: "linear-gradient(to bottom, #f5c518, #d4a800, #f5c518)",
    pattern:  "dashes",
    glow:     "rgba(245,197,24,0.35)",
  },
  Orange: {
    gradient: "linear-gradient(to bottom, #e8650a, #ff8c42, #e8650a)",
    pattern:  "dashes",
    glow:     "rgba(232,101,10,0.45)",
  },
  Green:  {
    gradient: "linear-gradient(to bottom, #1e4a1e, #3a8c3a, #1e4a1e)",
    pattern:  "double",
    glow:     "rgba(58,140,58,0.30)",
  },
  Blue:   {
    gradient: "linear-gradient(to bottom, #0f2548, #2255aa, #0f2548)",
    pattern:  "double",
    glow:     "rgba(34,85,170,0.40)",
  },
  Brown:  {
    gradient: "linear-gradient(to bottom, #3e2210, #7a4520, #3e2210)",
    pattern:  "dots",
    glow:     "rgba(122,69,32,0.35)",
  },
  Black:  {
    gradient: "linear-gradient(to bottom, #111, #555, #c9a84c, #555, #111)",
    pattern:  "zigzag",
    glow:     "rgba(201,168,76,0.30)",
  },
};

// ── Vertical belt panel (left side of card) ───────────────────────────────────
function BeltPanel({ belt, height }: { belt: Belt; height: number }) {
  const panel = BELT_PANEL[belt];
  const W = 6; // panel width px

  // Pattern overlay SVG — encoded inline
  ...
  return (
    <div style={{
      ...
      boxShadow:   `2px 0 12px 0 ${panel.glow}`,
      overflow:    "hidden",
    }}>
      {/* Pattern overlay */}
      {panel.pattern !== "solid" && (
      ...
      {/* Black belt gold spark travelling down */}
      {belt === "Black" && (
        <div style={{
          position:              "absolute",
          left:                  0,
          ...
          animationName:         "panel-spark",
          animationDuration:     "2.4s",
          animationTimingFunction:"ease-in-out",
          animationIterationCount:"infinite",
        }} />
      )}
      {/* Yellow/Orange animated bright band */}
...
// ── Animated belt avatar — rings ONLY around the profile circle ───────────────
function BeltAvatar({
  firstName, lastName, belt, photo,
}: {
  firstName: string; lastName: string; belt: Belt; photo?: string;
}) {
  const bc   = BELT_COLORS[belt];
  const aura = BELT_AURA[belt];
  const SIZE = 64; // avatar diameter
    ...
    <>
      <style>{`
        ...
        @keyframes aura-ember {
          ...
          40%  { opacity:0.6; transform:translate(-50%,-50%) scale(1.05) rotate(4deg);  }
          70%  { opacity:1;   transform:translate(-50%,-50%) scale(0.97) rotate(-3deg); }
          100% { opacity:0.9; transform:translate(-50%,-50%) scale(1)    rotate(0deg);  }
        }
        @keyframes aura-breathe {
          0%,100% { opacity:0.6; transform:translate(-50%,-50%) scale(1);    }
          50%      { opacity:0.2; transform:translate(-50%,-50%) scale(1.09); }
        }
        @keyframes aura-ripple {
          0%   { opacity:0.75; transform:translate(-50%,-50%) scale(1);    }
          100% { opacity:0;    transform:translate(-50%,-50%) scale(1.30); }
        }
        @keyframes aura-earthpulse {
          0%,100% { opacity:0.55; transform:translate(-50%,-50%) scale(1);    filter:blur(0px); }
          50%      { opacity:0.9;  transform:translate(-50%,-50%) scale(1.05); filter:blur(1px); }
        }
        @keyframes aura-rotate {
          0%   { transform:translate(-50%,-50%) rotate(0deg);   }
          100% { transform:translate(-50%,-50%) rotate(360deg); }
        }
        @keyframes aura-counter {
          0%   { transform:translate(-50%,-50%) rotate(0deg);   opacity:0.35; }
          50%  { opacity:0.15; }
          100% { transform:translate(-50%,-50%) rotate(-360deg); opacity:0.35; }
        }
        @keyframes sparks-orbit {
          0%   { transform:rotate(0deg)   translateX(34px) scale(1);   opacity:0.9; }
          50%  { transform:rotate(180deg) translateX(34px) scale(0.4); opacity:0.35; }
          100% { transform:rotate(360deg) translateX(34px) scale(1);   opacity:0.9; }
        }
        @keyframes panel-spark {
          0%   { top: -30px; }
          100% { top: 110%;  }
        }
      `}</style>

      {/* Wrapper — tight around avatar only, no overflow into card */}
      <div style={{
        position:       "relative",
        width:          `${SIZE + 16}px`,
        height:         `${SIZE + 16}px`,
        ...
        {/* ── Main animated ring — just outside avatar ── */}
        <div style={{
          position:     "absolute",
          top: "50%", left: "50%",
          width:        `${SIZE + 10}px`,
          height:       `${SIZE + 10}px`,
          borderRadius: "50%",
          // Black belt gets conic dashes, others get solid ring
          border: belt === "Black" ? "2px solid transparent" : `2px solid ${aura.ringColor}`,
          background: belt === "Black"
            ? `conic-gradient(${aura.ringColor} 0deg 55deg, transparent 55deg 110deg, ${aura.ringColor} 110deg 165deg, transparent 165deg 220deg, ${aura.ringColor} 220deg 275deg, transparent 275deg 330deg, ${aura.ringColor} 330deg 360deg)`
            : "transparent",
          WebkitMask: belt === "Black"
            ? `radial-gradient(farthest-side, transparent calc(100% - 3px), #fff calc(100% - 3px))`
            : "none",
          boxShadow: `0 0 8px 1px ${aura.glowColor}`,
          animationName:           aura.animation,
          animationDuration:       belt === "White"  ? "3s"
                                 : belt === "Yellow" ? "2s"
                                 : belt === "Orange" ? "1.8s"
                                 : belt === "Green"  ? "4s"
                                 : belt === "Blue"   ? "1.4s"
                                 : belt === "Brown"  ? "3.5s"
                                 : "4s",
          animationTimingFunction: belt === "Blue" || belt === "Black" ? "linear" : "ease-in-out",
          animationIterationCount: "infinite",
        }} />

        {/* ── Blue ripple: second ring, same position, offset timing ── */}
        {belt === "Blue" && (
          <div style={{
            position:     "absolute",
            top: "50%", left: "50%",
        ...
        {/* ── Black belt counter-rotate ring ── */}
        {belt === "Black" && (
          <div style={{
            position:     "absolute",
            top: "50%", left: "50%",
        ...
        {/* ── Black belt 3 orbiting spark dots ── */}
        {belt === "Black" && [0, 1, 2].map(i => (
          <div key={i} style={{
            ...
            animationName:           "sparks-orbit",
            animationDuration:       `${2.6 + i * 0.5}s`,
        ...
        <div style={{
          ...
          height:       `${SIZE + 4}px`,
          borderRadius: "50%",
          boxShadow:    `inset 0 0 6px 2px ${aura.glowColor}`,
  ...
  return (
    <span style={{
      ...
      <span style={{
        ...
        background: bc.text, opacity: 0.6,
      }} />
      {belt}
    </span>
  );
...
function StudentCard({ student }: { student: Student }) {
  return (
    <div style={{
      ...
      borderRadius: "4px",
      padding:      "18px 20px 18px 26px", // extra left padding for panel
      display:      "flex",
      gap:          "16px",
      position:     "relative",
    ...
    >
      {/* PUBG-style vertical belt panel — left edge */}
      <BeltPanel belt={student.belt} height={0 /* height:100% via CSS */} />

      {/* Champion crown badge — top-right corner */}
      {student.isChampion && (
        <div style={{
          position:      "absolute",
          top:           "10px",
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ display:"flex", alignItems:"center", gap:"10px", flexWrap:"wrap", marginBottom:"6px" }}>
          <span style={{
            ...
            fontWeight: 600, color: "#e8e8e8", letterSpacing: "0.5px",
          }}>
        ...
        <div style={{ display:"flex", alignItems:"center", gap:"8px", flexWrap:"wrap", marginBottom:"10px" }}>
          ...
          <span style={{ fontFamily:"var(--font-montserrat)", fontSize:"11px", color:"#555" }}>
            Age {student.age}
          </span>
        ...
        <div style={{ display:"flex", gap:"16px", flexWrap:"wrap" }}>
          ...
          <div style={{ marginLeft:"auto", display:"flex", alignItems:"center", gap:"5px" }}>
            <span style={{ fontFamily:"var(--font-montserrat)", fontSize:"10px", color:"#444", letterSpacing:"0.5px" }}>
              SENSEI
            </span>
            <span style={{ fontFamily:"var(--font-cormorant)", fontSize:"13px", color:"#888", fontStyle:"italic" }}>
              {student.sensei}
            </span>
          </div>
        </div>
...
function Stat({ label, value }: { label: string; value: number }) {
  return (
    <div style={{ display:"flex", flexDirection:"column", gap:"1px" }}>
      <span style={{ fontFamily:"var(--font-cinzel)", fontSize:"16px", fontWeight:700, color:"#ccc", lineHeight:1 }}>
        {value}
      </span>
      <span style={{ fontFamily:"var(--font-montserrat)", fontSize:"9px", color:"#444", letterSpacing:"1.5px", textTransform:"uppercase" }}>
        {label}
      </span>
    </div>
  );
  ...
  const filtered = useMemo(() => {
    return SAMPLE_STUDENTS.filter(s => {
      ...
      const matchSearch =
        ...
        s.sensei.toLowerCase().includes(q);
